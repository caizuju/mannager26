    ...
    <el-table :data="userList" border style="width: 100%">
      ...
      <el-table-column prop="mg_state" label="用户状态" width="180">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.mg_state" @change='state(scope)' active-color="#13ce66" inactive-color="#ff4949"></el-switch>
        </template>
      </el-table-column>
      <el-table-column prop="address" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="editUser(scope.row)" plain></el-button>
          <el-button
            type="danger"
            icon="el-icon-delete"
            size="mini"
...
export default {
  ...
  methods: {
   ...
   async state(data){
      ...
      //调用改变用户状态的接口
      let res  = await this.$axios.put(`users/${data.row.id}/state/${data.row.mg_state}`);
      console.log(res);
    },
    //添加用户
  addUser(){
